...
int main() {
    ...
    // }
            ...
            // Enable 핀 고정 (Enable = Active)
        // G1 = HIGH, G2A = LOW, G2B = LOW
        gpioWrite(PIN_EN, 1);

int pairs[][2] = {{1,2}, {2,3}, {3,1}};  // LED 번호
while (1) {
    for (int p = 0; p < 3; p++) {
        int a = pairs[p][0], b = pairs[p][1];
        // 첫 번째 LED
        gpioWrite(PIN_A, (a-1)&1);
        gpioWrite(PIN_B, ((a-1)>>1)&1);
        gpioWrite(PIN_C, ((a-1)>>2)&1);
        gpioWrite(PIN_EN, 1);
        gpioDelay(2000);  // 매우 짧게

        // 두 번째 LED
        gpioWrite(PIN_A, (b-1)&1);
        gpioWrite(PIN_B, ((b-1)>>1)&1);
        gpioWrite(PIN_C, ((b-1)>>2)&1);
        gpioWrite(PIN_EN, 1);
        gpioDelay(500000);  // 매우 짧게
    }
}
