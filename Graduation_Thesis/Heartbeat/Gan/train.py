...
def main():
    ...
    minority_features = torch.tensor(
        X_train_scaled[y_train == 1], dtype=torch.float32
    ).to(DEVICE)
    minority_loader = DataLoader(
        TensorDataset(minority_features), batch_size=GAN_BATCH_SIZE, shuffle=True
    )
    ...
    for epoch in range(1, GAN_EPOCHS + 1):
        for i, (real_samples,) in enumerate(minority_loader):
            ...
            real_loss = adversarial_loss(
                discriminator(real), torch.ones(real.size(0), 1, device=DEVICE)
            )
            ...
            g_loss = adversarial_loss(
                discriminator(gen_imgs), torch.ones(real.size(0), 1, device=DEVICE)
            )
        ...
        if epoch % (GAN_EPOCHS // 10) == 0:
            print(
                f"GAN Epoch {epoch}/{GAN_EPOCHS} | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f}"
            )
    ...
    train_dataset = TensorDataset(
        torch.tensor(X_train_resampled).unsqueeze(1), torch.tensor(y_train_resampled)
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        val_loss /= val_total
        print(
            f"Classifier Epoch {epoch}/{EPOCHS} | Train Loss: {train_loss:.4f}, Acc: {train_accuracy:.2f}% | Val Loss: {val_loss:.4f}, Acc: {val_accuracy:.2f}%"
        )
