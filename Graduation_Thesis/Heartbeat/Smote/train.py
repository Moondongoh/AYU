...
MODEL_SAVE_PATH = r"D:\MDO\heartbeat\beat\SMOTE\model\smote_model60.pt"
SCALER_SAVE_PATH = r"D:\MDO\heartbeat\beat\SMOTE\model\smote_scaler60.pkl"
# -- 하이퍼파라미터 ---
SEED = 42
EPOCHS = 60
BATCH_SIZE = 256
...
def main():
    set_seed(SEED)
    print(f"SMOTE 모델 학습을 시작합니다. Device: {DEVICE}")
    ...
    print(f"SMOTE 적용 전 훈련 데이터 shape: {X_train_scaled.shape}")
    print(f"SMOTE 적용 후 훈련 데이터 shape: {X_train_resampled.shape}")
    print(f"검증 데이터 shape: {X_val_scaled.shape}")
    print(f"SMOTE 적용 후 라벨 분포: {np.bincount(y_train_resampled)}")
