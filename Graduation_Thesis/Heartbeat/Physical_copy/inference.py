...
# 2. 물리적 복제 모델
MODEL_PATH = r"D:\MDO\heartbeat\beat\Physical_copy\model\physical_copy_model60.pt"
SCALER_PATH = r"D:\MDO\heartbeat\beat\Physical_copy\model\physical_copy_scaler60.pkl"
...
def evaluate():
    ...
    try:
        ...
        print(
            f"⚠️ 경고: 스케일러 파일을 찾을 수 없습니다 ({SCALER_PATH}). 스케일링 없이 진행합니다."
        )
        X_test_scaled = X_test

    test_dataset = TensorDataset(
        torch.tensor(X_test_scaled).unsqueeze(1), torch.tensor(y_test)
    )
    ...
    try:
        ...
        print(
            f"❌ 에러: 모델 파일을 찾을 수 없습니다 ({MODEL_PATH}). 평가를 중단합니다."
        )
        return
